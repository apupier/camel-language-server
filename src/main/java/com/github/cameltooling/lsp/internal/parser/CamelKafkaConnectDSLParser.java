...
public class CamelKafkaConnectDSLParser extends ParserFileHelper {
	...
	public String getCamelComponentUri(String line, int characterPosition) {
		if (new CamelKafkaConnectUtility().isInsideACamelUri(line, characterPosition)) {
			Properties properties = new Properties();
			try {
				properties.load(new StringReader(line));

				String camelComponentURI = properties.getProperty(CamelKafkaConnectUtility.CAMEL_SOURCE_URL);
				if (camelComponentURI != null) {
					return camelComponentURI;
				} else {
					return properties.getProperty(CamelKafkaConnectUtility.CAMEL_SINK_URL);
				}
			} catch (IOException e) {
	...
	public String getCorrespondingMethodName(TextDocumentItem textDocumentItem, int lineNumber) {
		...
		if (line.startsWith(CamelKafkaConnectUtility.CAMEL_SINK_URL)) {
			return "to";
		} else if(line.startsWith(CamelKafkaConnectUtility.CAMEL_SOURCE_URL)) {
			return "from";
		}
		return null;
	}
